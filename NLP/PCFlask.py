...
We then initialize a trie, insert each of our entries into it.
"""       
        
possibs = pd.read_csv(r"C:\Users\Asus\Downloads\python-challenge-master\python-challenge-master\test_files\6500titles.csv")
...
"""
We define the route which should be input so that we can autocomplete our search,
which in this case would be "http://127.0.0.1:5000/what%20we%20want%20to%20search".
Any spaces in our search must be replaced by %20 as shown in the example.
"""

@app.route('/<search>')
def index(search):        
    return ';\n'.join(trie.complete(search))
